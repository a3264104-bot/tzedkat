            ...
            <div key={l.id} className="card p-4">
              <div className="flex flex-wrap justify-between items-start gap-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {/* §229: 🔶 סגירה **ללקוחות בלבד** — מיידית.
                      
                      🐛 מה שהיה: השלב הזה קרה רק אוטומטית, בשעה
                      שנקבעה מראש. מנהל שרצה לסגור **עכשיו** (הספק
                      סגר הזמנות, נגמר המלאי) היה חייב לערוך את
                      שעת הסגירה - ארבע קליקים ובורר תאריכים.
                      
                      ⚠️ הכפתור פשוט קובע את closeDate ל"עכשיו".
                      אותו מנגנון בדיוק, בלחיצה אחת.
                      
                      ⚠️ מוצג רק כשהמכירה **עדיין פתוחה ללקוחות**:
                      אחרי שנסגרה אין מה לסגור שוב. */}
                  {l.status === "ACTIVE" &&
                    !(l.closeDate && new Date() > new Date(l.closeDate)) && (
                      <button
                        onClick={async () => {
                          if (
                            !window.confirm(
                              `לסגור את "${l.name}" ללקוחות עכשיו?\n\n` +
                                "לקוחות לא יוכלו להזמין יותר — באתר ובטלפון.\n\n" +
                                "✅ אתה והנציגים תמשיכו להזין הזמנות כרגיל."
                            )
                          )
                            return;
                          // ⚠️ ISO ולא toLocaleString: השרת שומר UTC,
                          // ושליחת מחרוזת מקומית הייתה נשמרת שגוי
                          // ב-3 שעות (§193).
                          await api(`/api/admin/pricelists/${l.id}`, {
                            method: "PATCH",
                            body: JSON.stringify({
                        ...
                        className="btn-ghost btn-sm text-orange-800 border-orange-300"
                      >
                        🔶 סגור ללקוחות
                      </button>
                    )}
                  ...
                  {/* §145: שליחת אקסל - רק ממכירה פעילה שאינה
                      לנציגים בלבד. */}
                  {/* §230: 🐛 הכפתור נשאר אחרי שהמכירה נסגרה ללקוחות.
                      
                      שליחת מחירון באקסל (§145) היא פעולה של **לפני**
                      המכירה: הלקוח מקבל קובץ, ממלא כמויות, ומחזיר.
                      אחרי שההרשמה נסגרה אין לו מה לעשות איתו - הוא
                      ימלא, ישלח, וההזמנה תידחה.
                      
                      ⚠️ והגרוע יותר: הוא ישב בשורה אחת עם "סגירה
                      סופית", ולחיצה בטעות שולחת מיילים למאות
                      לקוחות בלי דרך חזרה. */}
                  {/* §230: מפריד — שליחת מיילים אינה שינוי מצב.
                      
                      ⚠️ קו אנכי ולא רווח: הוא נראה גם כשהכפתורים
                      נשברים לשורה שנייה בנייד, ורווח לא. */}
                  {l.status === "ACTIVE" &&
                    !l.agentOnly &&
                    !(l.closeDate && new Date() > new Date(l.closeDate)) && (
                      <span className="w-px h-6 bg-zinc-300 mx-1 self-center" />
                    )}
                  {l.status === "ACTIVE" &&
                    !l.agentOnly &&
                    !(l.closeDate && new Date() > new Date(l.closeDate)) && (
