...
/**
 * §287: הכהיית גוון באחוז נתון.
 *
 * למה זה נדרש: הצבע למוצר והפס לסירוגין רצו על אותו תא. במקום
 * שאחד ידרוס את השני, השורה האפורה מקבלת את **אותו** צבע מוצר,
 * רק כהה יותר.
 *
 * ⚠️ 6% הוא המינימום שנראה לעין ולא משנה את זהות הגוון. יותר
 * מזה והנציג חושב שזה מוצר אחר.
 *
 * @param argb צבע בפורמט AARRGGBB של ExcelJS
 * @param amount 0..1 — כמה להכהות
 */
function darken(argb: string, amount: number): string {
  // ⚠️ שני התווים הראשונים הם אלפא ונשארים כפי שהם.
...
function buildDistributionSheet(
  ...
  // §287: הצבעים חזרו — לפי בקשת הנציגים.
  //
  // §285 ביטל אותם כי הדפים מודפסים בשחור-לבן. בפועל חלק
  // מהנציגים כן מדפיסים בצבע, והם דיווחו שזה עזר להם לזהות
  // מוצר במבט.
  //
  // ⚠️ מה שנשמר מ-§285: הצבע **לא מבטל את פסי השורות**. הפס
  // מוחל על השורה כולה, והצבע רק על התאים עם מוצרים - כך
  // ששתי המערכות עובדות יחד במקום להתנגש.
    ...
    chunks.forEach((chunk, ci) => {
      ...
      for (let i = 0; i < ITEMS_PER_ROW; i++) {
        ...
        if (it) {
          ...
          // §287: 🎨 הצבע חזר — ומשתלב עם הפסים.
          //
          // §161 נתן צבע לכל מוצר. §285 ביטל אותו כי הוא דרס
          // את פסי השורות. עכשיו שניהם עובדים יחד:
          //
          //   שורה לבנה  → הצבע המקורי של המוצר
          //   שורה אפורה → אותו צבע, **כהה ב-6%**
          //
          // ⚠️ ההכהיה שומרת על שתי המערכות: הנציג רואה שהמוצר
          // זהה (אותה משפחת גוון), וגם שהשורה שונה. בהדפסה
          // שחור-לבן ההפרש נשאר כהבדל בעוצמת האפור.
          const base = colorMap.get(name) || "FFFFFBEF";
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: stripe ? darken(base, 0.06) : base },
          };
        } else if (stripe) {
