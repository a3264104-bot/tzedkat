...
// §53: אילו פריטים דורשים שקילה.
//
// 🐛 הבאג שתוקן: הבדיקה הייתה `unit === "קרטון" || isSingle` - כלומר
// רק קרטונים ובודדים. מוצר ארוז שנמכר ביחידות ("בקר טחון 500 ג'",
// unit="יחידה", isSingle=false) לא עמד באף תנאי, ו*נעלם מרשימת
// השקילה לגמרי*: המנהל לא ידע שצריך לשקול אותו, המחיר הסופי של
// ההזמנה לא נקבע, וההזמנה נתקעה בלי חיוב.
//
// זה גם הסביר סתירה שנראתה במסכים: הדשבורד ספר הזמנות שממתינות
// לשקילה, ומסך המשקלים הציג "אין משקלים ממתינים" - הפריטים היו שם,
// פשוט סוננו החוצה.
//
// הכלל הנכון: כל פריט דורש שקילה, למעט בודדים שנמכרים ביחידות -
// שם הכמות היא מספר יחידות במחיר קבוע, ואין מה לשקול.
function needsWeighing(item: { unit: string; isSingle: boolean }): boolean {
  const isUnitPricedSingle =
    item.isSingle && (item.unit === "יחידה" || item.unit === "יחידות");
  return !isUnitPricedSingle;
}
