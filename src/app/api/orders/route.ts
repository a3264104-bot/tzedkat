...
export async function POST(req: Request) {
  ...
  try {
    ...
    // §60: לקוח מזומן לא מזמין באתר בעצמו - רק דרך נציג.
    // הגדרת מזומן משמעה שהגבייה מתבצעת פיזית בחלוקה מול הנציג, ואין
    // למערכת דרך לגבות ממנו הזמנת אתר. הדרך שלו להזמין לבד: להוסיף
    // כרטיס - save-token מעביר אותו לאשראי אוטומטית, ואז המסלול נפתח.
    // הבדיקה לפני בדיקת הטוקן, כדי שההודעה תסביר את הסיבה האמיתית
    // ולא "חסר כרטיס" גנרי.
    if (!placedByAgentId && role === "CUSTOMER" && customer.paymentPreference === "CASH") {
      return NextResponse.json(
        {
          error:
            "החשבון מוגדר לתשלום במזומן דרך נציג. להזמנה עצמאית באתר יש להוסיף כרטיס אשראי - לאחר אימות הכרטיס תוכל להזמין כרגיל.",
          code: "CASH_CUSTOMER",
        },
        { status: 403 }
      );
