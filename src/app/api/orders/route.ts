...
export async function POST(req: Request) {
  ...
  try {
    ...
    // למכירה של לפני חודש.
    // §227: **שני שלבי סגירה**, לא אחד.
    //
    // שלב 1 — עברה שעת הסגירה, המכירה עדיין ACTIVE:
    //   לקוחות חסומים · נציגים ומנהל ממשיכים.
    //   זה הזמן שבו מגיעות בקשות בטלפון ובחלוקה.
    //
    // שלב 2 — הסטטוס שונה ל-CLOSED:
    //   כולם חסומים. המכירה נסגרה סופית.
    //
    // 🐛 מה שהיה (§206): רק המנהל עבר את שלב 1. הנציג שעמד
    // בחלוקה מול לקוח שלא הזמין נחסם, ונאלץ להתקשר למנהל.
    //
    // ⚠️ הנציג מוגבל ללקוחות שלו ממילא (§55), ולכן הפתיחה
    // כאן אינה מרחיבה את מה שהוא רואה - רק את מתי.
    const isAdminOverride = role === "ADMIN" || !!placedByAgentId;
