...
function Modal({
  ...
  async function create() {
    ...
    // בא לפתור.
    // §307: 📞 **טלפון חובה** — כמו אצל הנציג.
    //
    // הטלפון הוא מזהה הכניסה הראשי (§71). בלעדיו הלקוח לא יכול
    // להתחבר, לא לאפס סיסמה, ולא לקבל שיחה ביום החלוקה.
    //
    // ⚠️ הבדיקה נבנתה קודם אצל הנציג בלבד, והמנהל נשאר בלי -
    // כלומר אותה תקלה בדיוק, רק במסך אחר.
    // ⚠️ במסך המנהל השדה נקרא phone, ולא q כמו אצל הנציג.
    const phoneDigits = (phone || "").trim().replace(/\D/g, "");
    if (phoneDigits.length < 9) {
      setError("יש להזין מספר טלפון תקין. הטלפון הוא מזהה הכניסה של הלקוח.");
      return;
    }
