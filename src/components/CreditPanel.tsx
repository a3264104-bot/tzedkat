...
// §363: 🐛 "יתרת חוב קודם" **הוסר** מהרשימה.
//
// חוב ממכירה קודמת אינו חיוב נוסף על ההזמנה הזו — הוא כסף
// שנגבה עבור מכירה אחרת. הרישום כ-extraCharge גרם ל:
//   • בקרת המכירה ספרה אותו כהכנסה מהמכירה הזו (§325 הפריד
//     appliedDebt, לא extraCharge)
//   • ההצלבה מול תעודות הספק הראתה עודף שאיש לא הבין
//
// ⚠️ המקום הנכון: פאנל "💸 רישום חוב" (§263) על הלקוח. הוא
// עובר להזמנה בשקילה (appliedDebt), נגבה בחיוב, ומופרד בדוחות.
...
export function CreditPanel({
  ...
  return (
    <div className={`${L.bg} border-2 ${L.borderStrong} rounded-xl p-3 space-y-2.5`}>
      ...
      {/* §363: ⚠️ הקליד "חוב" בסיבה — מפנים לפאנל הנכון.
          
          לא חוסמים (יש מקרי קצה), אבל מסבירים למה זה לא
          המקום. */}
      {isCharge && /חוב|יתרה קודמת|מכירה קודמת/.test(reason) && (
        <div className="rounded-lg bg-amber-50 border border-amber-300 px-3 py-2 text-[11px] text-amber-900 leading-relaxed">
          ⚠️ <b>חוב ממכירה קודמת?</b> יש לרשום אותו ב-
          <b>💸 רישום חוב</b> בכרטיס הלקוח — לא כאן. כך הוא ייגבה
          בחיוב ויופרד בדוחות מהכנסות המכירה הזו.
        </div>
      )}
