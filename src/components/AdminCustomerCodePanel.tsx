...
export function AdminCustomerCodePanel({
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-zinc-50 border border-blue-200 rounded-lg p-3 space-y-2.5">
      ...
      {/* §121: ההודעה על "הסיסמה הישנה" הוסרה.
          
          היא נועדה לתקופת המעבר, כשהיו לקוחות ותיקים שנרשמו לפני
          שהקודים הוצגו. המסד אופס מאז ואין לקוחות כאלה - ההודעה
          רק בלבלה, כי היא מרמזת על מסלול התחברות שלקוח חדש לא
          מכיר ולא קיים אצלו.
          
          במקומה: מה שבאמת רלוונטי - שהלקוח לא יוכל להיכנס לאתר
          עד שיופק לו קוד. */}
      {/* §122: ההודעה תלויה במצב האמיתי.
          
          🐛 מה שהיה: "אינו יכול להיכנס לאתר" הוצג גם ללקוח שנרשם
          בעצמו ובחר סיסמה - כלומר נכנס מצוין. ההודעה הייתה פשוט
          לא נכונה עבורו, והיא גם הציעה פעולה מיותרת.
      ...
      {!exists && !code && !passwordPlain && (
        ...
        >
          {hasPassword ? (
            <>
              ללקוח אין קוד, והוא נכנס לאתר בסיסמה שבחר בהרשמה. הפקת קוד
              תאפשר לו גם לשמוע אותו במערכת הטלפונית.
            </>
          ) : (
            <>
              ⚠️ ללקוח אין קוד ואין סיסמה, ולכן <b>אין לו דרך להיכנס</b>.
              הקוד מופק אוטומטית כשנקבע לו אמצעי תשלום, או ידנית בכפתור
              למעלה.
            </>
          )}
        </p>
