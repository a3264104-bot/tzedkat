...
export function prompt(file: string, text: string): string {
  ...
  //
  // ⚠️⚠️ הפורמט הזה **מעולם לא אומת בשיחה אמיתית**. הוא נבחר לפי
  // השערה, ואם הוא שגוי - ימות מחפשים קובץ שאינו קיים, והלקוח
  // שומע שקט ארוך במקום הודעה. זה מתאים לתיאור "15 שניות של שקט
  // בין הקלטות" ולכך שהקלטות רבות "לא נשמעות".
  //
  // YEMOT_FILE_FORMAT מאפשר לנסות את הווריאציות בלי פריסת קוד:
