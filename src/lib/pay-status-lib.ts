...
export type PayStatus =
  // §19 - 8 סטטוסים חדשים
  | "PENDING"              // ברירת המחדל — טרם נשקל/חויב
...
export const PAY_STATUS_LABELS: Record<string, string> = {
  // §264: 🐛 התווית הייתה "ממתין להזנת אשראי" — והיא **שגויה**.
  //
  // PENDING הוא ברירת המחדל של **כל** הזמנה חדשה (@default
  // בסכמה), ואין לו שום קשר לכרטיס. 250 הזמנות עם כרטיס שמור
  // הוצגו כ"ממתינות להזנת אשראי", והמנהל חשב שיש בעיה אצל
  // כולם.
  //
  // ⚠️ הבלבול נחשף רק אחרי §256, שהוסיף PENDING למסך התשלומים.
  // עד אז אף אחד לא ראה את התווית הזו.
